...
/* SQL Keywords */
%token DEALLOCATE PARAMETERS INTERSECT TEMPORARY TIMESTAMP TIMESTAMPTZ TIMEZONE
...
%token LEFT LIKE LOAD LONG NULL PARTITION PLAN SHOW TEXT THEN TIME TIMETZ
...
| TIMETZ opt_time_precision { $$ = ColumnType{DataType::TIMETZ, 0, $2}; }
| TIME WITH TIMEZONE opt_time_precision { $$ = ColumnType{DataType::TIMETZ, 0, $4}; }
| TIMESTAMP { $$ = ColumnType{DataType::DATETIME}; }
| TIMESTAMPTZ { $$ = ColumnType{DataType::DATETIMETZ}; }
| TIMESTAMP WITH TIMEZONE { $$ = ColumnType{DataType::DATETIMETZ}; }
